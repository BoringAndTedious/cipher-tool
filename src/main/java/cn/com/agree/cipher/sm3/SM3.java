...
public class SM3 {
	...
	public static final int DIGEST_LENGTH = 32;   // bytes
	...
	public int doFinal(byte[] out) {
		finish();
		intToBigEndian(V, out);
		return DIGEST_LENGTH;
	}

	private static void intToBigEndian(int[] ns, byte[] bs) {
		for (int i = 0, j = 0; i < ns.length; ++i, j += 4) {
			intToBigEndian(ns[i], bs, j);
		}
	}
	...
	private static abstract class DigestAlgorithmProvider extends AlgorithmProvider {
