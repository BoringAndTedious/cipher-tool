...
public class SM4 {
    ...
    private byte[] padding(byte[] input, int mode) {
        ...
        if (mode == SM4_ENCRYPT) {
            ...
            String inputHex = ByteUtil.byteToHex(input)+ "80";
            StringBuffer stringBuffer =new StringBuffer(inputHex);
            for (int i = 0; i <p-1 ; i++) {
                stringBuffer.append("00");
            }
            ret= ByteUtil.hexToByte(stringBuffer.toString());
            ...
            String inputHex = ByteUtil.byteToHex(input);
            int i = inputHex.lastIndexOf("80");
            String substring = inputHex.substring(0, i);
            ret= ByteUtil.hexToByte(substring);
        }
        return ret;
    }
